<div class="container">
    ...
    <?php if(!empty($_SESSION)){
        if($_SESSION['role'] === 'admin') { ?><a href="?page=new-yearbook" class="homepage-new-student">Ajouter un nouveau yearbook</a><?php }} ?>
    <div class="homepage-year-display">
        ...
        <?php if(!empty($_SESSION)){
            if($_SESSION['role'] === 'admin') { ?><a href="?page=delate-yearbook&userId=<?= $data['id']?>"  class="yearbook-delete"><ion-icon name="trash-outline" class="yearbook-mod-icon"></ion-icon>delete</a><?php } ?>
        </div>
        <?php }} ?>
    </div>
</div>
